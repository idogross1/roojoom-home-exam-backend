...
const { getSolution } = require('./responseService')
...
async function add(bug) {
  const solution = getSolution(bug)
  bug.solution = solution
  var sqlCmd = `INSERT INTO bug (user_id, date, serial_number, free_text, first_indicator, second_indicator, third_indicator, response) 
                VALUES ("${bug.userId}",
                        "${Date.now()}",
                        "${bug.serialNumber}",
...
function remove(bugId) {
  ...
  return DBService.runSQL(query).then((okPacket) => (okPacket.affectedRows === 1 ? okPacket : Promise.reject(new Error(`No bug deleted - bug id ${bugId}`))))
}
